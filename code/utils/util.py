...
def tensor2img(tensor, out_type=np.float32, min_max=(0, 1)):
    ...
    if n_dim == 4:
        n_img = len(tensor)
        img_np = tensor.permute(0, 2, 3, 1).cpu().data.numpy()
        img_np = np.minimum(np.maximum(img_np,0),1)
        # img_np = make_grid(tensor, nrow=int(math.sqrt(n_img)), normalize=False).numpy()
        # img_np = np.transpose(img_np[[2, 1, 0], :, :], (1, 2, 0))  # HWC, BGR
    elif n_dim == 3 and tensor.shape[0]>3:
        n_img = tensor.shape[0]
        img_np = tensor.cpu().data.numpy()
        img_np = np.minimum(np.maximum(img_np,0),1)
        return [img_np[i,:,:] for i in range(n_img)]
    elif n_dim == 3:
    ...
    return img_np[0,:,:,:].astype(out_type)
